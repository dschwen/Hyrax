...
AuxAMRNucleationProbability::computeValue()
{
  ...
  //get mesh area in 2D or volume in 3D - this will have whatever dimension or nondimensionalized scale
  //you supply in input file
  Real vol;
  if (_mesh_dimension == 2)
    vol = _current_elem_volume*_2D_mesh_height;
  else vol = _current_elem_volume;

  //Make sure the dimensions of nucleation rate calculation match
  //up with the rest of the simulation!
  //Note that if using MeshSolutionModify executioner, mesh
  //refinement is applied AFTER the values are computed.
  //Therefore nucleation probability with AMR is going to be
  //useless output
