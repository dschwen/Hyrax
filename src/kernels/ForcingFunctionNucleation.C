...
ForcingFunctionNucleation::ForcingFunctionNucleation(const std::string & name, InputParameters parameters)
...
Real
ForcingFunctionNucleation::computeQpResidual()
{
  ...
  if(_t >= _start_time && _t < _end_time)
  {
    ...
    // Currently, sharp interface only!
    if(distance <=_radius)
      return UserForcingFunction::computeQpResidual();
    else
      return 0.0;
  }
  else
